...
import time
import argparse
import eval_MutateMethodInvExpr
...
class Train(object):
    def __init__(self):
        ...
        self.batcher = Batcher(config_MutateMethodInvExpr.train_data_path, self.vocab, mode='train',
                               batch_size=config_MutateMethodInvExpr.batch_size,
                               single_pass=False)  

        time.sleep(15)

        if not os.path.exists(config_MutateMethodInvExpr.log_root):
            os.mkdir(config_MutateMethodInvExpr.log_root)
        train_dir = os.path.join(config_MutateMethodInvExpr.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_MutateMethodInvExpr.lr_coverage if config_MutateMethodInvExpr.is_coverage else config_MutateMethodInvExpr.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_MutateMethodInvExpr.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_MutateMethodInvExpr.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_MutateMethodInvExpr.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_MutateMethodInvExpr.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_MutateMethodInvExpr.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_MutateMethodInvExpr.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}".format(config_MutateMethodInvExpr.mlp_weight,
                                                                  config_MutateMethodInvExpr.nmt_weight))
        print("\n")
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_MutateMethodInvExpr.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_MutateMethodInvExpr.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_MutateMethodInvExpr.epochs, config_MutateMethodInvExpr.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_MutateMethodInvExpr.max_iterations, args.model_file_path)  
