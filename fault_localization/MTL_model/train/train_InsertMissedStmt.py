...
from data_util import config_InsertMissedStmt
...
use_cuda = config_InsertMissedStmt.use_gpu and torch.cuda.is_available()
...
class Train(object):
    def __init__(self):
        ...
        # Reading data
        self.vocab = Vocab(config_InsertMissedStmt.vocab_path, config_InsertMissedStmt.vocab_size)
        self.batcher = Batcher(config_InsertMissedStmt.train_data_path, self.vocab, mode='train',
                               batch_size=config_InsertMissedStmt.batch_size,
                               single_pass=False)  

        time.sleep(15)

        if not os.path.exists(config_InsertMissedStmt.log_root):
            os.mkdir(config_InsertMissedStmt.log_root)
        train_dir = os.path.join(config_InsertMissedStmt.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_InsertMissedStmt.lr_coverage if config_InsertMissedStmt.is_coverage else config_InsertMissedStmt.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_InsertMissedStmt.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_InsertMissedStmt.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_InsertMissedStmt.max_dec_steps)):  
            ...
            step_loss = -torch.log(gold_probs + config_InsertMissedStmt.eps)
            if config_InsertMissedStmt.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_InsertMissedStmt.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_InsertMissedStmt.mlp_weight
        nmt_weight = config_InsertMissedStmt.nmt_weight

        new_loss = mlp_loss * mlp_weight + nmt_loss * nmt_weight
        loss = new_loss
        loss.backward()

        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_InsertMissedStmt.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_InsertMissedStmt.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_InsertMissedStmt.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_InsertMissedStmt.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}  \n".format(config_InsertMissedStmt.mlp_weight,
                                                                  config_InsertMissedStmt.nmt_weight))
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_InsertMissedStmt.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_InsertMissedStmt.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_InsertMissedStmt.epochs, config_InsertMissedStmt.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_InsertMissedStmt.max_iterations, args.model_file_path)  
