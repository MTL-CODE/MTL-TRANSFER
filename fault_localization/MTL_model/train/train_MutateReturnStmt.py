...
from data_util import config_MutateReturnStmt
...
use_cuda = config_MutateReturnStmt.use_gpu and torch.cuda.is_available()
print("is use GPU ?", use_cuda)


class Train(object):
    def __init__(self):
        self.vocab = Vocab(config_MutateReturnStmt.vocab_path, config_MutateReturnStmt.vocab_size)
        self.batcher = Batcher(config_MutateReturnStmt.train_data_path, self.vocab, mode='train',
                               batch_size=config_MutateReturnStmt.batch_size,
                               single_pass=False)  
        time.sleep(15)

        if not os.path.exists(config_MutateReturnStmt.log_root):
            os.mkdir(config_MutateReturnStmt.log_root)
        train_dir = os.path.join(config_MutateReturnStmt.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_MutateReturnStmt.lr_coverage if config_MutateReturnStmt.is_coverage else config_MutateReturnStmt.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_MutateReturnStmt.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_MutateReturnStmt.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_MutateReturnStmt.max_dec_steps)):  
            ...
            step_loss = -torch.log(gold_probs + config_MutateReturnStmt.eps)
            if config_MutateReturnStmt.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_MutateReturnStmt.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_MutateReturnStmt.mlp_weight
        nmt_weight = config_MutateReturnStmt.nmt_weight
        ...
        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_MutateReturnStmt.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_MutateReturnStmt.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_MutateReturnStmt.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_MutateReturnStmt.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}".format(config_MutateReturnStmt.mlp_weight,
                                                                  config_MutateReturnStmt.nmt_weight))
        print("\n")
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_MutateReturnStmt.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_MutateReturnStmt.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_MutateReturnStmt.epochs, config_MutateReturnStmt.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_MutateReturnStmt.max_iterations, args.model_file_path) 
