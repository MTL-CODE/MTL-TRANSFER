...
from data_util import config_MoveStmt
...
use_cuda = config_MoveStmt.use_gpu and torch.cuda.is_available()
print("is use GPU ?", use_cuda)


class Train(object):
    def __init__(self):
        self.vocab = Vocab(config_MoveStmt.vocab_path, config_MoveStmt.vocab_size)
        self.batcher = Batcher(config_MoveStmt.train_data_path, self.vocab, mode='train',
                               batch_size=config_MoveStmt.batch_size,
                               single_pass=False)  

        
        time.sleep(15)

        if not os.path.exists(config_MoveStmt.log_root):
            os.mkdir(config_MoveStmt.log_root)
        train_dir = os.path.join(config_MoveStmt.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_MoveStmt.lr_coverage if config_MoveStmt.is_coverage else config_MoveStmt.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_MoveStmt.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_MoveStmt.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_MoveStmt.max_dec_steps)):  
            y_t_1 = dec_batch[:, di]  
            ...
            step_loss = -torch.log(gold_probs + config_MoveStmt.eps)
            if config_MoveStmt.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_MoveStmt.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_MoveStmt.mlp_weight
        nmt_weight = config_MoveStmt.nmt_weight
        ...
        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_MoveStmt.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_MoveStmt.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_MoveStmt.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_MoveStmt.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{} \n".format(config_MoveStmt.mlp_weight,
                                                                  config_MoveStmt.nmt_weight))
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_MoveStmt.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_MoveStmt.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_MoveStmt.epochs, config_MoveStmt.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_MoveStmt.max_iterations, args.model_file_path) 
