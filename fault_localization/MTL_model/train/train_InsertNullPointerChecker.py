...
import time
import argparse
import eval_InsertNullPointerChecker
...
from data_util import config_InsertNullPointerChecker
...
use_cuda = config_InsertNullPointerChecker.use_gpu and torch.cuda.is_available()
...
class Train(object):
    def __init__(self):
        ...
        # Reading data
        self.vocab = Vocab(config_InsertNullPointerChecker.vocab_path, config_InsertNullPointerChecker.vocab_size)
        self.batcher = Batcher(config_InsertNullPointerChecker.train_data_path, self.vocab, mode='train',
                               batch_size=config_InsertNullPointerChecker.batch_size,
                               single_pass=False) 

        time.sleep(15)

        if not os.path.exists(config_InsertNullPointerChecker.log_root):
            os.mkdir(config_InsertNullPointerChecker.log_root)
        train_dir = os.path.join(config_InsertNullPointerChecker.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_InsertNullPointerChecker.lr_coverage if config_InsertNullPointerChecker.is_coverage else config_InsertNullPointerChecker.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_InsertNullPointerChecker.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_InsertNullPointerChecker.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_InsertNullPointerChecker.max_dec_steps)):  
            ...
            step_loss = -torch.log(gold_probs + config_InsertNullPointerChecker.eps)
            if config_InsertNullPointerChecker.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_InsertNullPointerChecker.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_InsertNullPointerChecker.mlp_weight
        nmt_weight = config_InsertNullPointerChecker.nmt_weight
        ...
        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_InsertNullPointerChecker.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_InsertNullPointerChecker.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_InsertNullPointerChecker.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_InsertNullPointerChecker.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}  \n".format(config_InsertNullPointerChecker.mlp_weight,
                                                                  config_InsertNullPointerChecker.nmt_weight))
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_InsertNullPointerChecker.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_InsertNullPointerChecker.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_InsertNullPointerChecker.epochs, config_InsertNullPointerChecker.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_InsertNullPointerChecker.max_iterations, args.model_file_path) 
