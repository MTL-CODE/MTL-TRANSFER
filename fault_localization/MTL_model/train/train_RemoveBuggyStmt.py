...
from data_util import config_RemoveBuggyStmt
...
use_cuda = config_RemoveBuggyStmt.use_gpu and torch.cuda.is_available()
print("is use GPU ?", use_cuda)


class Train(object):
    def __init__(self):
        self.vocab = Vocab(config_RemoveBuggyStmt.vocab_path, config_RemoveBuggyStmt.vocab_size)
        self.batcher = Batcher(config_RemoveBuggyStmt.train_data_path, self.vocab, mode='train',
                               batch_size=config_RemoveBuggyStmt.batch_size,
                               single_pass=False)  

        time.sleep(15)

        if not os.path.exists(config_RemoveBuggyStmt.log_root):
            os.mkdir(config_RemoveBuggyStmt.log_root)
        train_dir = os.path.join(config_RemoveBuggyStmt.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_RemoveBuggyStmt.lr_coverage if config_RemoveBuggyStmt.is_coverage else config_RemoveBuggyStmt.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_RemoveBuggyStmt.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_RemoveBuggyStmt.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_RemoveBuggyStmt.max_dec_steps)):  
            ...
            step_loss = -torch.log(gold_probs + config_RemoveBuggyStmt.eps)
            if config_RemoveBuggyStmt.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_RemoveBuggyStmt.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_RemoveBuggyStmt.mlp_weight
        nmt_weight = config_RemoveBuggyStmt.nmt_weight

        new_loss = mlp_loss * mlp_weight + nmt_loss * nmt_weight
        loss = new_loss
        loss.backward()

        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_RemoveBuggyStmt.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_RemoveBuggyStmt.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_RemoveBuggyStmt.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_RemoveBuggyStmt.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}".format(config_RemoveBuggyStmt.mlp_weight,
                                                                  config_RemoveBuggyStmt.nmt_weight))
        print("\n")
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_RemoveBuggyStmt.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_RemoveBuggyStmt.Evaluate(train_processor.model)
            one_epoach_eval_acc, nmt_epoch_eval_loss = eval_processor.run_eval()
            print("eval====eval_mlp_acc is %.2f, eval_all_loss is %f ..........this epoch run_time is %d." %
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_RemoveBuggyStmt.epochs, config_RemoveBuggyStmt.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_RemoveBuggyStmt.max_iterations, args.model_file_path) 
