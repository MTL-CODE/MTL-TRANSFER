...
from data_util import config_MutateVariable
...
use_cuda = config_MutateVariable.use_gpu and torch.cuda.is_available()
print("is use GPU ?", use_cuda)


class Train(object):
    def __init__(self):
        self.vocab = Vocab(config_MutateVariable.vocab_path, config_MutateVariable.vocab_size)
        self.batcher = Batcher(config_MutateVariable.train_data_path, self.vocab, mode='train',
                               batch_size=config_MutateVariable.batch_size,
                               single_pass=False)  

        time.sleep(15)

        if not os.path.exists(config_MutateVariable.log_root):
            os.mkdir(config_MutateVariable.log_root)
        train_dir = os.path.join(config_MutateVariable.log_root, 'save_model')
    ...
    def setup_train(self, model_file_path=None):
        ...
        initial_lr = config_MutateVariable.lr_coverage if config_MutateVariable.is_coverage else config_MutateVariable.lr  # 0.15
        self.optimizer = Adagrad(params, lr=initial_lr,
                                 initial_accumulator_value=config_MutateVariable.adagrad_init_acc)
        ...
        if model_file_path is not None:
            ...
            if not config_MutateVariable.is_coverage:
    ...
    def train_one_batch(self, batch):
        ...
        step_losses = []
        for di in range(min(max_dec_len, config_MutateVariable.max_dec_steps)): 
            ...
            step_loss = -torch.log(gold_probs + config_MutateVariable.eps)
            if config_MutateVariable.is_coverage:
                step_coverage_loss = torch.sum(torch.min(attn_dist, coverage), 1)
                step_loss = step_loss + config_MutateVariable.cov_loss_wt * step_coverage_loss
        ...
        mlp_weight = config_MutateVariable.mlp_weight
        nmt_weight = config_MutateVariable.nmt_weight
        ...
        self.norm = clip_grad_norm_(self.model.encoder.parameters(), config_MutateVariable.max_grad_norm)
        clip_grad_norm_(self.model.decoder.parameters(), config_MutateVariable.max_grad_norm)
        clip_grad_norm_(self.model.reduce_state.parameters(), config_MutateVariable.max_grad_norm)
        clip_grad_norm_(self.model.mlp.parameters(), config_MutateVariable.max_grad_norm)
    ...
    def Epochs_Train(self, epochs, n_iters, model_file_path=None):
        ...
        best_mlp_acc = 0
        best_nmt_loss = 10000
        print("the proportion of mlp and nmt is.....{}/{}".format(config_MutateVariable.mlp_weight,
                                                                  config_MutateVariable.nmt_weight))
        print("\n")
        ...
        for i in range(epochs):
            ...
            mlp_acc_epoch, new_epoch_loss, nmt_epoch_loss, mlp_epoch_loss = train_processor.trainIters(
                config_MutateVariable.max_iterations, args.model_file_path)
            ...
            eval_processor = eval_MutateVariable.Evaluate(train_processor.model)
...
if __name__ == '__main__':
    ...
    train_processor.Epochs_Train(config_MutateVariable.epochs, config_MutateVariable.max_iterations,
                                 args.model_file_path)

    # train_processor.trainIters( config_MutateVariable.max_iterations, args.model_file_path) 
