...
def parse_model_output_file():    
	...
	for group_index in range(1, 11):
		with open("../ranking_task/run_model/result/{}.txt".format(group_index), "r") as file:
...
if __name__ == "__main__":
	for k in range(10):
		...
		for i in range(10):
			...
			# status = os.system("python3 pipeline_group.py")
			assert status == 0
			...
			for group_index in range(1,11):
				...
				#
				#status = os.system("python3 train.py {}".format(group_index))
				assert status == 0
			...
			result_dict['top1_total_list'].append(top1_total)
			result_dict['top3_total_list'].append(top3_total)
			result_dict['top5_total_list'].append(top5_total)
			result_dict['MFR_list'].append(MFR)
			result_dict['MAR_list'].append(MAR)
