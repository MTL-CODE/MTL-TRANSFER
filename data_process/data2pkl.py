...
def work(faultType):
    ...
    faultTypePath = os.path.join('../data/data-diff', faultType)
    ...
    for i in range(0, len(dataToken), 2):
        if (len(dataToken[i]['beforefix']) <= maxBeforeFixLength) and (
                len(dataToken[i]['afterfix']) <= maxAfterFixLength) and (
                len(dataToken[i + 1]['beforefix']) <= maxBeforeFixLength) and (
                len(dataToken[i + 1]['afterfix']) <= maxAfterFixLength):
